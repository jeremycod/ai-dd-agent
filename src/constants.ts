export const PROMPT =
  ...
  '\n   - **`OFFER_PRICE` (Offer Pricing Issues):** Problems specifically related to the price of an offer or product, including incorrect pricing displayed, pricing not updating as expected, discrepancies in price across environments, or issues with promotions/discounts affecting the final price. **Keywords/Phrases for this category: "incorrect price", "missing price", "price discrepancy", "price not updating", "promotional price issue", "ad-tier price", "cost of", "rate for".**' + // Added more keywords
  ...
  "\n3. **Identify Key Information:** Extract relevant details like entity IDs, entity type (e.g., 'offer', 'campaign'), and any specific timeframes. **For any issue related to price, ensure you identify the `offerId` and set `entityType` as `offer`.**" + // Refined hint
  ...
  '\n   - **If the environment is not clearly specified by the user and is essential for further investigation or tool execution (e.g., for `getDatadogLogs` or `getOfferPrice` tool), you MUST ask for clarification before proceeding.** For example, "Which environment (production, staging, or development) is this entity in?"' + // Refined environment instruction
  '\n   - **Ensure the provided environment is one of the valid options**: `production`, `staging`, or `development`.' +
  '\n5. **Strategize Tool Use:** Based on the information gathered, the nature of the suspected issue, and the **validated environment**, determine which of your available tools are best suited for investigation. **For `OFFER_PRICE` issues, prioritize using the `getOfferPrice` tool.** For log analysis, use `getDatadogLogs`.' + // Direct tool hint
