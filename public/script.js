...
function addMessage(content, sender, isMarkdown = false, addFeedback = false, showNumberIcon = false) {
    ...
    if (sender === 'agent') {
        ...
        avatar.innerHTML = '<img src="https://via.placeholder.com/24/6C63FF/FFFFFF?text=AI" alt="AI Avatar">';
    ...
    if (isMarkdown) {
        ...
        if (sender === 'agent') { // Only for agent messages
    ...
    if (sender === 'agent' && addFeedback && !isFirstAgentMessage) {
        ...
        feedbackContainer.querySelectorAll('.feedback-button').forEach(button => {
            button.addEventListener('click', function() {
                ...
                const commentInput = commentSection.querySelector('.feedback-comment-input');
                commentInput.focus();
...
async function sendMessage() {
    ...
    typingIndicator.innerHTML = 'Genie+ is typing<span>.</span><span>.</span><span>.</span>';
    ...
    try {
        ...
        if (response.ok) {
            ...
            const showNumber = messageToDisplay.includes("Learning from Production Data");
...
document.addEventListener('DOMContentLoaded', () => {
    const existingInitialAgentMessage = messagesDiv.querySelector('.agent-message-wrapper');
    if (existingInitialAgentMessage) {
        existingInitialAgentMessage.remove();
    }
    addMessage("Hello! I'm Genie+ Assistant. How can I help you today?", 'agent', false, false, false);
    messagesDiv.scrollTop = messagesDiv.scrollHeight;
});